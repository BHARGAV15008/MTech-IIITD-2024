...
void connectToPrintServer(unsigned char *kap, const unsigned char *nonce, const char *filename) {
    ...
    struct sockaddr_in addr = {.sin_family=AF_INET, .sin_port=htons(PRN_PORT), .sin_addr.s_addr=inet_addr("127.0.0.1")};
...
int main(int argc, char *argv[]) {
    ...
    struct sockaddr_in kdcAddr = {.sin_family=AF_INET, .sin_port=htons(KDC_PORT), .sin_addr.s_addr=inet_addr("127.0.0.1")};
    ...
    connectToPrintServer(kap, nonce, filename);
