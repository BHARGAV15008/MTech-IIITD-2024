...
from . import ZeroKnowledge_Convert  # Import convert module from the parent package
from typing import Union  # Import Union for type hints
from .ZeroKnowledge_Algorithms import Hash_types  # Import Hash_types from the parent package
...
def hash_data(*values: Union[str, bytes, bytearray, int, Point], algorithm="sha3_256") -> bytes:
    ...
    return Hash_types[algorithm](b"".join(map(ZeroKnowledge_Convert.to_bytes, values))).digest()  # Compute hash of concatenated values
...
def hash_numeric(*values: Union[str, bytes, bytearray, int, Point], alg="sha3_256") -> int:
    ...
    """
    return ZeroKnowledge_Convert.bytes_to_int(hash_data(*values, algorithm=alg))  # Convert hash digest to integer form
