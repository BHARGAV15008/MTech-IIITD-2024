# # We have this type of datasets train.csv
# id,CustomerId,Surname,CreditScore,Geography,Gender,Age,Tenure,Balance,NumOfProducts,HasCrCard,IsActiveMember,EstimatedSalary,Exited
# 0,15570087.0,Chigozie,571.0,Germany,Female,42.0,4.0,127290.61,1.0,1.0,1.0,25669.1,0.0
# 1,15809837.0,Chubb,558.0,France,Male,38.0,2.0,0.0,1.0,1.0,1.0,138849.06,0.0
# 2,15766776.0,Ch'ien,644.0,France,Female,44.0,3.0,0.0,1.0,1.0,0.0,121408.46,1.0
# 3,15649536.0,Chikelu,714.0,France,Female,27.0,6.0,0.0,2.0,1.0,0.0,121151.1,0.0
# 4,15637644.0,Cremonesi,676.0,Spain,Female,33.0,8.0,0.0,2.0,0.0,1.0,170392.59,0.0
# 5,15662908.0,Y?,561.0,France,Male,30.0,1.0,0.0,2.0,1.0,0.0,44335.54,0.0
# 6,15773852.0,Oluchukwu,645.0,France,Female,41.0,1.0,0.0,1.0,1.0,0.0,81452.29,1.0
# 7,15747534.0,Chinomso,607.0,France,Female,34.0,10.0,0.0,2.0,1.0,0.0,111342.66,0.0
# 8,15757895.0,DeRose,687.0,France,Female,35.0,5.0,99610.92,1.0,1.0,1.0,107815.31,0.0
# 9,15592578.0,Walker,642.0,Germany,Female,42.0,3.0,104015.54,1.0,1.0,0.0,159334.93,1.0
# 10,15783629.0,Rizzo,678.0,France,Male,38.0,9.0,0.0,2.0,0.0,0.0,179631.85,0.0

# # We have this type of datasets test.csv
# id,CustomerId,Surname,CreditScore,Geography,Gender,Age,Tenure,Balance,NumOfProducts,HasCrCard,IsActiveMember,EstimatedSalary
# 15000,15767954.0,Smith,701.0,France,Female,33.0,10.0,0.0,2.0,1.0,0.0,62402.38
# 15001,15641110.0,Ch'ang,757.0,France,Male,32.0,10.0,104469.58,1.0,1.0,1.0,63795.8
# 15002,15589496.0,Chukwujekwu,613.0,France,Male,34.0,4.0,0.0,2.0,1.0,0.0,136983.77
# 15003,15777892.0,Nkemjika,684.0,France,Female,41.0,8.0,0.0,2.0,1.0,1.0,147090.9
# 15004,15652914.0,Lucciano,648.0,Spain,Male,38.0,2.0,0.0,2.0,1.0,1.0,54495.82
# 15005,15632576.0,Chidiegwu,663.0,Germany,Female,44.0,1.0,127847.86,1.0,1.0,0.0,103726.71
# 15006,15734999.0,Chukwudi,624.0,France,Female,51.0,8.0,0.0,3.0,1.0,0.0,187985.85
# 15007,15813916.0,Chukwufumnanya,627.0,France,Male,31.0,5.0,0.0,2.0,1.0,1.0,161465.31
# 15008,15681180.0,Chiemenam,464.0,Germany,Female,43.0,0.0,124576.65,1.0,1.0,0.0,80190.36
# 15009,15596713.0,Chiang,718.0,France,Male,36.0,2.0,0.0,2.0,1.0,0.0,162643.15
# 15010,15773971.0,T'ien,686.0,France,Male,35.0,2.0,0.0,2.0,0.0,1.0,110114.19
# 15011,15780142.0,Mazzi,641.0,Germany,Female,51.0,1.0,102827.44,1.0,1.0,1.0,159418.1



# # 1. HandoutCrossValidator
# from sklearn.model_selection import train_test_split

# class HandoutCrossValidator:
#     def __init__(self, test_size=0.2, random_state=42):
#         self.test_size = test_size
#         self.random_state = random_state

#     def split(self, X, y):
#         X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=self.test_size, random_state=self.random_state)
#         return X_train, X_test, y_train, y_test


# # 2. KFoldCrossValidator
# from sklearn.model_selection import KFold

# class KFoldCrossValidator:
#     def __init__(self, n_splits=5):
#         self.n_splits = n_splits

#     def split(self, X, y):
#         kf = KFold(n_splits=self.n_splits)
#         for train_index, test_index in kf.split(X):
#             X_train, X_test = X[train_index], X[test_index]
#             y_train, y_test = y[train_index], y[test_index]
#             yield X_train, X_test, y_train, y_test


# # 3. StratifiedKFoldCrossValidator
# from sklearn.model_selection import StratifiedKFold

# class StratifiedKFoldCrossValidator:
#     def __init__(self, n_splits=5):
#         self.n_splits = n_splits

#     def split(self, X, y):
#         skf = StratifiedKFold(n_splits=self.n_splits)
#         for train_index, test_index in skf.split(X, y):
#             X_train, X_test = X[train_index], X[test_index]
#             y_train, y_test = y[train_index], y[test_index]
#             yield X_train, X_test, y_train, y_test


# # 4. GroupKFoldCrossValidator
# from sklearn.model_selection import GroupKFold

# class GroupKFoldCrossValidator:
#     def __init__(self, n_splits=5):
#         self.n_splits = n_splits

#     def split(self, X, y, groups):
#         gkf = GroupKFold(n_splits=self.n_splits)
#         for train_index, test_index in gkf.split(X, y, groups=groups):
#             X_train, X_test = X[train_index], X[test_index]
#             y_train, y_test = y[train_index], y[test_index]
#             yield X_train, X_test, y_train, y_test


# # 5. TimeSeriesSplitCrossValidator
# from sklearn.model_selection import TimeSeriesSplit

# class TimeSeriesSplitCrossValidator:
#     def __init__(self, n_splits=5):
#         self.n_splits = n_splits

#     def split(self, X, y):
#         tscv = TimeSeriesSplit(n_splits=self.n_splits)
#         for train_index, test_index in tscv.split(X):
#             X_train, X_test = X[train_index], X[test_index]
#             y_train, y_test = y[train_index], y[test_index]
#             yield X_train, X_test, y_train, y_test


# # 6. RepeatedKFoldCrossValidator
# from sklearn.model_selection import RepeatedKFold

# class RepeatedKFoldCrossValidator:
#     def __init__(self, n_splits=5, n_repeats=10):
#         self.n_splits = n_splits
#         self.n_repeats = n_repeats

#     def split(self, X, y):
#         rkf = RepeatedKFold(n_splits=self.n_splits, n_repeats=self.n_repeats)
#         for train_index, test_index in rkf.split(X):
#             X_train, X_test = X[train_index], X[test_index]
#             y_train, y_test = y[train_index], y[test_index]
#             yield X_train, X_test, y_train, y_test


# # 7. TestTrainSplitCrossValidator
# from sklearn.model_selection import train_test_split

# class TestTrainSplitCrossValidator:
#     def __init__(self, test_size=0.2, random_state=42):
#         self.test_size = test_size
#         self.random_state = random_state

#     def split(self, X, y):
#         X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=self.test_size, random_state=self.random_state)
#         return X_train, X_test, y_train, y_test




import numpy as np
import pandas as pd
from sklearn.model_selection import train_test_split, KFold, StratifiedKFold, GroupKFold, TimeSeriesSplit, RepeatedKFold

class BaseCrossValidator:
    def __init__(self):
        pass

    def _validate_input(self, X, y):
        if not isinstance(X, (np.ndarray, pd.DataFrame)):
            raise ValueError("X should be a numpy array or pandas DataFrame")
        if y is not None and not isinstance(y, (np.ndarray, pd.Series)):
            raise ValueError("y should be a numpy array or pandas Series")
        if y is not None and len(X) != len(y):
            raise ValueError("X and y must have the same length")

class HandoutCrossValidator(BaseCrossValidator):
    def __init__(self, test_size=0.2, random_state=42):
        super().__init__()
        self.test_size = test_size
        self.random_state = random_state

    def split(self, X, y=None):
        """
        Split the data into train and test sets.
        
        Parameters:
        X (array-like): The input samples
        y (array-like, optional): The target values

        Returns:
        X_train, X_test, y_train, y_test
        """
        self._validate_input(X, y)
        return train_test_split(X, y, test_size=self.test_size, random_state=self.random_state)

class KFoldCrossValidator(BaseCrossValidator):
    def __init__(self, n_splits=5, shuffle=True, random_state=None):
        super().__init__()
        self.n_splits = n_splits
        self.shuffle = shuffle
        self.random_state = random_state

    def split(self, X, y=None):
        """
        Generate indices to split data into training and test set.

        Parameters:
        X (array-like): The input samples
        y (array-like, optional): The target values

        Yields:
        X_train, X_test, y_train, y_test for each fold
        """
        self._validate_input(X, y)
        kf = KFold(n_splits=self.n_splits, shuffle=self.shuffle, random_state=self.random_state)
        for train_index, test_index in kf.split(X):
            X_train, X_test = X[train_index], X[test_index]
            y_train, y_test = (y[train_index], y[test_index]) if y is not None else (None, None)
            yield X_train, X_test, y_train, y_test

class StratifiedKFoldCrossValidator(BaseCrossValidator):
    def __init__(self, n_splits=5, shuffle=True, random_state=None):
        super().__init__()
        self.n_splits = n_splits
        self.shuffle = shuffle
        self.random_state = random_state

    def split(self, X, y):
        """
        Generate indices to split data into training and test set.

        Parameters:
        X (array-like): The input samples
        y (array-like): The target values

        Yields:
        X_train, X_test, y_train, y_test for each fold
        """
        self._validate_input(X, y)
        skf = StratifiedKFold(n_splits=self.n_splits, shuffle=self.shuffle, random_state=self.random_state)
        for train_index, test_index in skf.split(X, y):
            X_train, X_test = X[train_index], X[test_index]
            y_train, y_test = y[train_index], y[test_index]
            yield X_train, X_test, y_train, y_test

class GroupKFoldCrossValidator(BaseCrossValidator):
    def __init__(self, n_splits=5):
        super().__init__()
        self.n_splits = n_splits

    def split(self, X, y, groups):
        """
        Generate indices to split data into training and test set.

        Parameters:
        X (array-like): The input samples
        y (array-like): The target values
        groups (array-like): Group labels for the samples

        Yields:
        X_train, X_test, y_train, y_test for each fold
        """
        self._validate_input(X, y)
        if len(groups) != len(X):
            raise ValueError("groups must have the same length as X")
        gkf = GroupKFold(n_splits=self.n_splits)
        for train_index, test_index in gkf.split(X, y, groups=groups):
            X_train, X_test = X[train_index], X[test_index]
            y_train, y_test = y[train_index], y[test_index]
            yield X_train, X_test, y_train, y_test

class TimeSeriesSplitCrossValidator(BaseCrossValidator):
    def __init__(self, n_splits=5):
        super().__init__()
        self.n_splits = n_splits

    def split(self, X, y=None):
        """
        Generate indices to split data into training and test set.

        Parameters:
        X (array-like): The input samples
        y (array-like, optional): The target values

        Yields:
        X_train, X_test, y_train, y_test for each fold
        """
        self._validate_input(X, y)
        tscv = TimeSeriesSplit(n_splits=self.n_splits)
        for train_index, test_index in tscv.split(X):
            X_train, X_test = X[train_index], X[test_index]
            y_train, y_test = (y[train_index], y[test_index]) if y is not None else (None, None)
            yield X_train, X_test, y_train, y_test

class RepeatedKFoldCrossValidator(BaseCrossValidator):
    def __init__(self, n_splits=5, n_repeats=10, random_state=None):
        super().__init__()
        self.n_splits = n_splits
        self.n_repeats = n_repeats
        self.random_state = random_state

    def split(self, X, y=None):
        """
        Generate indices to split data into training and test set.

        Parameters:
        X (array-like): The input samples
        y (array-like, optional): The target values

        Yields:
        X_train, X_test, y_train, y_test for each fold in each repeat
        """
        self._validate_input(X, y)
        rkf = RepeatedKFold(n_splits=self.n_splits, n_repeats=self.n_repeats, random_state=self.random_state)
        for train_index, test_index in rkf.split(X):
            X_train, X_test = X[train_index], X[test_index]
            y_train, y_test = (y[train_index], y[test_index]) if y is not None else (None, None)
            yield X_train, X_test, y_train, y_test